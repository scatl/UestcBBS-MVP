...
class SelectBoardPresenter: BaseVBPresenter<SelectBoardView>() {
    ...
    fun getMainForumList() {
        boardModel.getForumList(SharePrefUtil.getToken(mView?.getContext()), SharePrefUtil.getSecret(mView?.getContext()),
            object : Observer<ForumListBean>() {
    ...
    fun getChildBoardList(fid: Int, fatherBoardName: String) {
        boardModel.getSubForumList(fid,
            ...
            SharePrefUtil.getSecret(mView?.getContext()), object : Observer<SubForumListBean>() {
    ...
    fun getClassification(boardId: Int) {
        boardModel.getSingleBoardPostList(1, 0,
            ...
            object : Observer<SingleBoardBean>() {
                override fun OnSuccess(singleBoardBean: SingleBoardBean) {
                    if (singleBoardBean.rs == ApiConstant.Code.SUCCESS_CODE) {
                        ...
                        mView?.onGetClassificationSuccess(singleBoardBean)
                    } else if (singleBoardBean.rs == ApiConstant.Code.ERROR_CODE) {
                        mView?.onGetClassificationError(singleBoardBean.head.errInfo)
                    }
                }

                override fun onError(e: ResponseThrowable) {
                    mView?.onGetClassificationError(e.message)
                }
