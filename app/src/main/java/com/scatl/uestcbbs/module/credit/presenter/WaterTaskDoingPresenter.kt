...
class WaterTaskDoingPresenter: BaseVBPresenter<WaterTaskDoingView>() {
    ...
    fun getDoingTaskList() {
        creditModel
            ...
            .subscribeEx(Observer<String>().observer {
                ...
                onError {
                    mView?.onGetDoingTaskError("获取任务失败：${it.message}")
                }
    ...
    fun deleteDoingTask(id: Int, position: Int) {
        creditModel
            .deleteDoingTask(id, SharePrefUtil.getForumHash(mView?.getContext()))
            .subscribeEx(Observer<String>().observer {
                onSuccess {
                    try {
                        val document = Jsoup.parse(it)
                ...
                onError {
                    mView?.onDeleteDoingTaskError("放弃任务失败：${it.message}")
                }
    ...
    fun getTaskAward(position: Int, id: Int) {
        creditModel
            ...
            .subscribeEx(Observer<String>().observer {
                ...
                onError {
                    mView?.onGetTaskAwardError("领取奖励失败：${it.message}")
                }
    ...
    fun checkLeftTime(id: Int, textView: TextView? = null) {
        creditModel
            ...
            .subscribeEx(Observer<String>().observer {
                ...
                onError {
                    mView?.onCheckLeftTimeError("查看剩余时间失败：${it.message}")
                }
