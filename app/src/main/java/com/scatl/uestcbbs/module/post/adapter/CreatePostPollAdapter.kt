...
class CreatePostPollAdapter: PreloadAdapter<String, ItemCreatePostPollBinding>() {
    ...
    override fun onBindViewHolder(holder: ViewBindingHolder<ItemCreatePostPollBinding>, position: Int, item: String?) {
        super.onBindViewHolder(holder, position, item)
        if (item == null) {
            return
        }
        holder.binding.itemCreatePostPollTextview.text = (holder.layoutPosition + 1).toString() + ". " + item
    }
}